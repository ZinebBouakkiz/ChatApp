...
class FriendsRecyclerAdapter: RecyclerView.Adapter<FriendsRecyclerAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        val ivFrined: ShapeableImageView = itemView.findViewById(R.id.ivfriend)
        ...
        fun bind(friend: Friend){
            ...
            // put friend.image in ivFrined
            if(friend.image.isNotEmpty()){
                Glide.with(itemView.context).load(friend.image).placeholder(R.drawable.photo_user).into(ivFrined)
            }
